...
void setup(void)
{
	...
	sei();
	//	(d) Display your student number, "n9950095", with nominal top-left
	//	corner at screen location (8,30). Left-aligned 10
	//	pixels below that, display the pre-scale factor that corresponds
	//	to a timer overflow period of approximately 0.5
	//	seconds.
	draw_string(8, 30, "n9950095", FG_COLOUR);
	draw_string(8, 40, "64", FG_COLOUR);

	// Keep the next instruction intact.
	show_screen();
}

//	(e) Create a volatile global variable called overflow_counter.
...
//	routine is called, overflow_counter should increment by 1.
ISR (TIMER1_OVF_vect) {
    overflow_counter++;
}
...
double get_elapsed_time() {
	return (overflow_counter * 65536.0 + TCNT1) * 64 / 8000000;
}
